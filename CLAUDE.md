...
| **Pivô Gemini/Claude Code** — Prospecção sem API paga | Prospecção IA trocou de provider (Claude → `geminiProvider.js`, Gemini Developer API/`gemini-flash-latest` + Google Search Grounding, free tier — não Vertex AI) pra eliminar dependência de crédito Anthropic; `claudeProvider.js` preservado como fallback, não apagado. Agente de Outbound ganhou um SEGUNDO caminho de execução, manual: `/outbound-weekly` (Claude Code local, autenticado por Claude Pro via OAuth — nunca `ANTHROPIC_API_KEY`) lê leads via ADC (`scripts/outbound-weekly-list.js`) e grava abordagens via ADC reaproveitando `lib/outbound/messages.js` (`scripts/outbound-weekly-write.js`) — a Cloud Function `generateOutboundMessage` (Claude API paga) continua existindo em paralelo, não removida. Validado com dados reais de produção (2 leads reais criados via Gemini, 3 abordagens reais geradas via Claude Code) antes do deploy — 92 testes automatizados, 0 falhas. | `portal-associativo/docs/roadmap/PIVO_GEMINI_CLAUDE_CODE_REPORT.md` |
...
- **Caminho alternativo, sem API paga** (Pivô Gemini/Claude Code): `/outbound-weekly` (slash command, `.claude/commands/outbound-weekly.md`) roda inteiramente local via Claude Code autenticado por Claude Pro (nunca `ANTHROPIC_API_KEY`) — `scripts/outbound-weekly-list.js` lê leads elegíveis via ADC (`gcloud auth application-default login`, nunca service account key em disco), o próprio Claude Code gera a abordagem (sem pesquisa web adicional nesta versão), `scripts/outbound-weekly-write.js` grava reaproveitando `lib/outbound/messages.js` (mesmo contrato `outboundMessages/{leadId}` da Cloud Function). Revisão humana/envio manual em `admin/outbound-ia.html` continuam idênticos, qualquer que seja o caminho de geração usado.
