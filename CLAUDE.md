...
| **Botão "Gerar Leads IA"** — disparo manual da Prospecção, achado crítico corrigido | Extensão pequena: botão em `admin/leads.html` reaproveita a callable `requestProspectingRun` já existente (mesma usada pelo "Executar agora" de `admin/prospeccao-ia.html`) — zero motor novo, `trigger:"manual"` (campo já existente) distingue do scheduler (`trigger:"scheduled"`). **Achado crítico real, não hipótese**: `getFunctions().taskQueue(...).enqueue()` (firebase-admin) falhava consistentemente com "Queue does not exist" mesmo com IAM correto — confirmado que a MESMA chamada funciona via REST puro ao Cloud Tasks. Corrigido substituindo por `lib/cloudTasksDispatch.js` (REST direto, `fetch()`, mesmo padrão do resto do código) em `enqueueProspectingRun`/`enqueueOutboundBatch`. Isso provavelmente significa que **o scheduler semanal nunca completou uma execução de verdade antes desta correção**. Achado secundário: `executeProspectingRun`/`executeOutboundBatch` (Gen2) rodam sob a service account default do Compute (`{project-number}-compute@developer.gserviceaccount.com`), não a `appspot` — só esta última tinha acesso aos secrets (`gemini-api-key`, `anthropic-api-key`, `email-user`, `email-password`); corrigido concedendo `roles/secretmanager.secretAccessor` também à SA do Compute. Também descoberto e corrigido: não existia nenhuma campanha `active` em produção (só campanhas de teste arquivadas) — criada a campanha oficial `nIyOtBX8JBzm0uZAWZIX` com o ICP documentado em `salesContext`. Validado com 2 execuções reais via a callable deployada de verdade (não só localmente): 3 leads (teste controlado, limite reduzido temporariamente) e depois 20 leads no limite normal. 118 testes automatizados, 0 falhas. | `portal-associativo/docs/roadmap/PIVO_GEMINI_CLAUDE_CODE_REPORT.md` |
